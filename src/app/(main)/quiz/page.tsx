...
export default function QuizDashboardPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Trigger generation (fire and forget - we poll for status)
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">{t("quizDashboardDesc")}</p>
      </div>
      ...
      {/* Quiz List */}
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-4">{t("myQuizzes")}</h2>
        {quizzes.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    {quiz.status === "READY" ? (
                      <Link href={`/quiz/${quiz.id}`} className="font-semibold text-gray-900 hover:text-blue-600 transition-colors block truncate">
                        {quiz.title}
                      </Link>
                    ) : (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-400">{t("noQuizzes")}</p>
          </div>
        )}
      </div>
    </div>
