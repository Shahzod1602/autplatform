  ...
  useEffect(() => {
    if (session) {
      fetch("/api/courses")
        .then((res) => res.json())
        .then(async (courses) => {
          // Get user enrollments by checking each course
          ...
          for (const course of courses) {
            try {
              ...
              // Check enrollment status from enrollments
              if (data.enrollments) {
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">{t("startToday")}</p>
      </div>
      ...
      <div className="mb-8">
        ...
        {enrollments.length > 0 ? (
            ...
            {enrollments.map((enrollment) => (
              ...
              >
                ...
                {enrollment.course.description && (
                  <p className="text-sm text-gray-500 mb-3 line-clamp-2">{enrollment.course.description}</p>
                )}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p className="text-gray-400 mb-4">{t("noEnrollments")}</p>
            <Link
              href="/courses"
